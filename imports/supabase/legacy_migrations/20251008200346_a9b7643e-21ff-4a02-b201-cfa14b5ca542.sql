-- Simplify API key generation using only built-in Postgres functions
CREATE OR REPLACE FUNCTION public.make_workspace_enterprise(target_workspace_id uuid)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Generate API key using built-in UUID functions
  random_part := replace(gen_random_uuid()::text || gen_random_uuid()::text, '-', '');
  api_key_plain := 'rant_' || substring(random_part, 1, 32);
  -- Use md5 for simple hashing (built-in function)
  api_key_hash := md5(api_key_plain || target_workspace_id::text);
