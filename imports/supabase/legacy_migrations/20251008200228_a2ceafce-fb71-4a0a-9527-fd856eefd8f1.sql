-- Use Postgres built-in gen_random_uuid instead of pgcrypto's gen_random_bytes
...
BEGIN
  ...
  -- Generate API key using built-in functions
  random_suffix := replace(gen_random_uuid()::text || gen_random_uuid()::text, '-', '');
  workspace_secret := 'rantir_enterprise_' || target_workspace_id::text;
  api_key_plain := 'rant_' || substring(random_suffix, 1, 32);
  api_key_hash := encode(hmac(api_key_plain::bytea, workspace_secret::bytea, 'sha256'), 'hex');
